import { GraphicAttributes, YearAttributes as YearAttributes } from "./types";
...
function _createInfoChart(attributes: GraphicAttributes) {
  ...
  bar.append("rect")
    ...
    .on("mousemove", (d, i: number) => {
      ...
      tooltip.html(d.value.toLocaleString() + " ha")
...
function _getData(attr: GraphicAttributes) {
  ...
  return yearCodes.map((e: YearAttributes) => {
    e.value = attr[e.field];
    return e;
  });
}
...
function _getTotal(attr: GraphicAttributes) {
  ...
  for (let i = 2001; i <= 2019; i++) {
    totalAttr += attr["tc_loss_ha_" + i.toString()];
  }
  return totalAttr;
  // return attr["total_loss"]
}
...
//     .attr("width", width)
//     .attr("height", height);
//   const bar = hueLegend.selectAll("tc_loss_ha_")
//     .data(yearCodes)
//     // .data(partyCodes)
//     .enter().append("g")
